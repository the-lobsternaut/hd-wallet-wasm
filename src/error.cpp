...
size_t curvePublicKeyCompressedSize(Curve curve) {
    switch (curve) {
        case Curve::SECP256K1:
            return 33;  // 1-byte prefix + 32-byte X coordinate
        case Curve::ED25519:
            return 32;  // Ed25519 public keys are always 32 bytes
        case Curve::P256:
            return 33;
        case Curve::P384:
            return 49;  // 1-byte prefix + 48-byte X coordinate
        case Curve::X25519:
...
size_t curvePublicKeyUncompressedSize(Curve curve) {
    switch (curve) {
        case Curve::SECP256K1:
            return 65;  // 1-byte prefix (0x04) + 32-byte X + 32-byte Y
        case Curve::ED25519:
            return 32;  // Ed25519 only has one format
        case Curve::P256:
            return 65;
        case Curve::P384:
            return 97;  // 1-byte prefix + 48-byte X + 48-byte Y
        case Curve::X25519:
            return 32;  // X25519 only has one format
        default:
            return 0;
    }
}
...
// =============================================================================
// Coin Type to String Conversion
// =============================================================================

/**
 * Convert CoinType enum to string name
 *
 * @param coin Coin type
 * @return Static string with coin name
 */
...
/**
 * Get the elliptic curve used by a coin type
 *
 * @param coin Coin type
 * @return Corresponding curve
 */
Curve coinTypeToCurve(CoinType coin) {
    switch (coin) {
        ...
        default:
            // Default to secp256k1 for unknown coins
            return Curve::SECP256K1;
    }
}
