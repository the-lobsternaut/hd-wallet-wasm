import{TrustLevel as L,scanBitcoinTrustTransactions as q,scanSolanaTrustTransactions as I,scanEthereumTrustTransactions as O,TrustLevelNames as N}from"./blockchain-trust-Bs_Cj1Ka.js";function C(s,e=12,t=8){return s?s.length<=e+t+3?s:`${s.slice(0,e)}...${s.slice(-t)}`:""}function M(s,e=10,t=6){return s?s.length<=e+t+3?s:`${s.slice(0,e)}...${s.slice(-t)}`:""}function B(s,e){switch(s){case"btc":return`https://blockstream.info/tx/${e}`;case"eth":return`https://etherscan.io/tx/${e}`;case"sol":return`https://solscan.io/tx/${e}`;default:return`https://blockstream.info/tx/${e}`}}function F(s){const t={btc:"BTC",eth:"ETH",sol:"SOL"}[s]||(s==null?void 0:s.toUpperCase())||"???";return`<span class="chain-badge chain-${s}">${t}</span>`}function U(s){const e=N[s]||"Unknown";return`<span class="trust-level-badge trust-level-${e.toLowerCase().replace(/\s+/g,"-")}">${e}</span>`}function D(s){switch(s){case"outbound":return'<span class="trust-direction" title="Outbound">&rarr;</span>';case"inbound":return'<span class="trust-direction" title="Inbound">&larr;</span>';case"mutual":return'<span class="trust-direction" title="Mutual">&harr;</span>';default:return'<span class="trust-direction">--</span>'}}function A(s){s.classList.remove("active"),setTimeout(()=>s.remove(),200)}function W(s,e,t){if(s.innerHTML="",!e||e.length===0){s.innerHTML='<div class="trust-empty">No trust relationships found.</div>';return}const r=document.createElement("div");r.className="trust-list";for(const i of e){const a=document.createElement("div");a.className="trust-row";const n=new Set(Array.isArray(t)?t:Object.values(t||{}).flat()),l=n.has(i.from),c=n.has(i.to),d=l&&c?"mutual":l?"outbound":c?"inbound":"outbound",g=d==="inbound"?i.from:i.to,f=i.chain||i.network||"btc",p=document.createElement("div");p.className="trust-row-header",p.innerHTML=`
    ...
    `,p.addEventListener("click",()=>{const m=a.classList.contains("expanded");r.querySelectorAll(".trust-row.expanded").forEach($=>$.classList.remove("expanded")),m||a.classList.add("expanded")}),a.appendChild(p),a.appendChild(T),r.appendChild(a)}s.appendChild(r)}function R(s){if(!s)return null;const e=s.trim();return/^(1|3)[a-km-zA-HJ-NP-Z1-9]{25,34}$/.test(e)||/^bc1[a-z0-9]{25,90}$/.test(e)?"btc":/^0x[0-9a-fA-F]{40}$/.test(e)?"eth":/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(e)?"sol":null}function _(s){const e=s.replace(/\r?\n /g,"").split(/\r?\n/),t={name:null,email:null,org:null,photo:null,keys:[],addresses:[]};for(const r of e){const i=r.indexOf(":");if(i===-1)continue;const a=r.substring(0,i).toUpperCase(),n=r.substring(i+1);if(a==="FN")t.name=n;else if(a.startsWith("EMAIL"))t.email=n;else if(a.startsWith("ORG"))t.org=n.replace(/;/g,", ");else if(a.startsWith("PHOTO")){if(a.includes("VALUE=URI")||n.startsWith("data:")||n.startsWith("http"))t.photo=n;else if(a.includes("ENCODING=B")||a.includes("ENCODING=b")){const l=a.match(/TYPE=(\w+)/i),c=l?l[1].toLowerCase():"jpeg";t.photo=`data:image/${c};base64,${n}`}}else if(a.startsWith("KEY")||a.startsWith("X-CRYPTO")||a.startsWith("X-KEY")){t.keys.push(n);const l=R(n);l&&t.addresses.push({address:n,chain:l})}}return t}const j=[{value:L.NEVER,name:"Never Trust",desc:"Block this address from all interactions",color:"#ef4444",border:"rgba(239, 68, 68, 0.4)"},{value:L.UNKNOWN,name:"Unknown",desc:"No opinion on this address yet",color:"#9ca3af",border:"rgba(107, 114, 128, 0.4)"},{value:L.MARGINAL,name:"Marginal",desc:"Somewhat trusted, proceed with caution",color:"#fbbf24",border:"rgba(245, 158, 11, 0.4)"},{value:L.FULL,name:"Full Trust",desc:"Highly trusted, verified relationship",color:"#6ee7b7",border:"rgba(16, 185, 129, 0.4)"},{value:L.ULTIMATE,name:"Ultimate",desc:"Your own address or absolute trust",color:"#a78bfa",border:"rgba(139, 92, 246, 0.4)"}];function G(s){let e=null;const t=document.createElement("div");t.className="modal trust-modal establish-trust-modal";const r=j.map((o,u)=>`
  ...
  `}function Y(s,e){let t=(s||[]).map((l,c)=>{var d;return{id:l.id||`rule-${c}`,type:l.type||"mutual_tx_count",params:{threshold:((d=l.params)==null?void 0:d.threshold)??0},resultLevel:l.resultLevel??L.MARGINAL,severity:l.severity||"info",description:l.description||""}});const r=document.createElement("div");r.className="modal trust-modal rules-modal";function i(){const l=t.map((c,d)=>z(c,d)).join("");r.innerHTML=`
      <div class="modal-glass">
        <div class="modal-header">
          <h3>Trust Rules</h3>
          <button class="modal-close">&times;</button>
    ...
    `,n()}function a(){r.querySelectorAll(".rule-row").forEach((c,d)=>{t[d]&&(t[d].type=c.querySelector(".rule-type").value,t[d].params.threshold=parseInt(c.querySelector(".rule-threshold").value,10)||0,t[d].resultLevel=parseInt(c.querySelector(".rule-result-level").value,10),t[d].severity=c.querySelector(".rule-severity").value)})}function n(){const l=()=>A(r);r.querySelector(".modal-close").addEventListener("click",l),r.querySelector("#rules-cancel").addEventListener("click",l),r.querySelector("#rules-add").addEventListener("click",()=>{a(),t.push({id:`rule-${Date.now()}`,type:"mutual_tx_count",params:{threshold:0},resultLevel:L.MARGINAL,severity:"info",description:""}),i()}),r.querySelectorAll(".rule-delete-btn").forEach(c=>{c.addEventListener("click",()=>{a();const d=parseInt(c.getAttribute("data-index"),10);t.splice(d,1),i()})}),r.querySelector("#rules-save").addEventListener("click",()=>{a(),e(t),l()})}document.body.appendChild(r),i(),requestAnimationFrame(()=>r.classList.add("active"))}async function J(s){const e=[];if(s.btc){const t=await q(s.btc);e.push(...t)}if(s.sol){const t=await I(s.sol);e.push(...t)}if(s.eth){const t=await O(s.eth);e.push(...t)}return e}function K(s,e){const t={exportDate:new Date().toISOString(),xpub:e||null,chainInfo:{btc:"Bitcoin mainnet",sol:"Solana mainnet-beta",eth:"Ethereum mainnet"},transactions:s||[]},r=JSON.stringify(t,null,2),i=new Blob([r],{type:"application/json"}),a=URL.createObjectURL(i),n=document.createElement("a");n.href=a,n.download=`trust-export-${Date.now()}.trust.json`,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(a)}function X(s){return new Promise((e,t)=>{if(!s){t(new Error("No file provided"));return}const r=new FileReader;r.onload=i=>{try{const a=JSON.parse(i.target.result);if(!a.transactions||!Array.isArray(a.transactions)){t(new Error("Invalid trust data: missing transactions array"));return}e(a.transactions)}catch(a){t(new Error(`Failed to parse trust data: ${a.message}`))}},r.onerror=()=>t(new Error("Failed to read file")),r.readAsText(s)})}export{K as exportTrustData,X as importTrustData,W as renderTrustList,J as scanAllTrustTransactions,G as showEstablishTrustModal,Y as showRulesModal,C as truncatePubkey,M as truncateTxHash};
